...
object AppointmentValidation {
    ...
    fun validate(
        ...
        if (appointmentType !in listOf("general", "doctor", "business", "personal", "meeting")) {
        ...
        if (reminderMinutesBefore !in 0..1440) {
            errors.add(
                ValidationError("reminderMinutesBefore", "Reminder minutes must be between 0 and 1440")
            )
        }
        ...
        if (isRecurring && recurrencePattern == null) {
            errors.add(ValidationError("recurrencePattern", "Recurring appointments must have a recurrence pattern"))
        }
        if (!isRecurring && recurrencePattern != null) {
            errors.add(ValidationError("recurrencePattern", "Non-recurring appointments cannot have a recurrence pattern"))
        }
